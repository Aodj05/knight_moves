...
class Knight
    ...
    def legal_move?(first, last)
        first_di = first[0] - last[0]
        second_di = first[1] - last[1]
        if first_di.abs == 2 && second_di.abs == 1 || first_di.abs == 1 && second_di.abs == 2
            true
        else
            false
        end 
...
def knight_moves(start, finish)
    ...
    puts "Success in : #{k_moves.length-1} moves. With this path:"
    k_moves.each { |move| print "#{move}\n"}
end
